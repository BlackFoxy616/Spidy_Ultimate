...
async def main():
   async with app:
     ...
     os.system(f"""yt-dlp  --downloader aria2c --download-archive dled.txt  --max-downloads 250 -o '%(title)s.%(ext)s' -f '(mp4)[height=?480]' --write-thumbnail --embed-metadata """ + link)
     #os.system(f"""./yt-dlp   --downloader aria2c  --download-archive dled.txt -I 700:701 -o '%(title)s.%(ext)s' -f '(mp4)[height=?480]' --write-thumbnail --embed-metadata """ + link)
